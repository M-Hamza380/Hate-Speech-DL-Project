...
class ModelTrainer:
    ...
    def spliting_data(self, csv_path):
        try:
            ...
            logging.info(f"Splitting the data into x & y")
            x = df[self.model_trainer_config.TWEET].astype(str)  # Ensure correct path
            y = df[self.model_trainer_config.LABEL]  # Ensure correct path
    ...
    def tokenizing(self, x_train):
        try:
            ...
            logging.info(f"Converting text into sequences: {sequences}")  # Fix typo: "test" to "text"
    ...
    def initiate_model_trainer(self) -> ModelTrainerArtifact:
        try:
            ...
            os.makedirs(os.path.dirname(self.model_trainer_config.TOKENIZER_PATH), exist_ok=True)
